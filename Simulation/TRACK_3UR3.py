...
class OuterArm:
    """IR_support UR3 placed where the old cylinders were, facing inward."""
    def __init__(self, env, cfg: RingCfg, theta_base: float, r_offset: float):
        self.cfg = cfg
        self.theta_base = float(theta_base)
        self.r_offset = float(r_offset)

        # Base outside the ring, rotated to face inward
        self.T_base = (
            SE3(cfg.center_x, cfg.center_y, 0.0)
            * SE3.Rz(theta_base)
            * SE3.Tx(cfg.radius + r_offset-0.20)
            * SE3.Rz(np.pi)
        )

        # Spawn the IR_support UR3 robot
        self.robot = UR3()
        self.robot.base = self.T_base
        env.add(self.robot)

    def angle(self) -> float:
        return self.theta_base

# --- NEW: a little manager for three arms ---
class ArmFleet:
    def __init__(self, env, cfg: RingCfg, k: int, r_margin: float):
        self.arms = []
        for j in range(k):
            th = 2 * np.pi * j / k
            self.arms.append(OuterArm(env, cfg, th, r_margin))

    def nearest_by_theta(self, theta: float):
        """Return the arm with smallest angular distance to theta."""
        best = None
        best_d = 1e9
        for arm in self.arms:
            d = min((theta - arm.angle()) % (2*np.pi),
                    (arm.angle() - theta) % (2*np.pi))
            if d < best_d:
                best_d, best = d, arm
        return best
...
class App:
    ...
    def add_outer_arms(self, cfg: RingCfg):
        # place arms a little outside the OUT radius
        r_margin = self.ring.out_offset + 0.10
        self.arms = ArmFleet(self.env, cfg, k=3, r_margin=r_margin)
